...
말투와 태도(Tone & Manner) 뿐만 아니라 사용자 분석에 있어서도 각 상황 별로 세세하게 분류하여 서비스를 다양하게 제공할 필요성을 느꼈다. <Data-Driven UX> 책을 읽으면서도 인종, 언어 등 제품을 이용하는 사용자의 세세한 환경까지 전부 고려해서 분류하고 분석해야 한다는 걸 알게 됐는데 이를 실제 사례로 풀어낸 과정을 보게 되어서 좋았다.
